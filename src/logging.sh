...
FEDORA_SETUP_STATE_DIR="$XDG_STATE_HOME/fedora-setup"
DEFAULT_LOG_DIR="$FEDORA_SETUP_STATE_DIR/logs"
...
init_logging() {
  ...
  # Now ensure FEDORA_SETUP_STATE_DIR exists
  if [[ ! -d "$FEDORA_SETUP_STATE_DIR" && "$LOG_DIR" != "/tmp/fedora-setup-logs" ]]; then
    echo "Creating application state directory: $FEDORA_SETUP_STATE_DIR"
    mkdir -p "$FEDORA_SETUP_STATE_DIR" 2>/dev/null || {
      echo "ERROR: Failed to create application state directory: $FEDORA_SETUP_STATE_DIR" >&2
